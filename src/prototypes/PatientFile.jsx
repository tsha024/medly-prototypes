...
import {
  ...
  AlertCircle, ShieldCheck, ShieldAlert, Save, CheckCircle2, X, ChevronLeft, FileDown, Loader2, History,
} from 'lucide-react';
...
export default function PatientFile({
  ...
  onSaveInsurance,             // reception: persist insurer / policy number
  ...
  const saveInsurance = () => {
    onSaveInsurance && onSaveInsurance({ insurer, policy });
    ...
    <div style={{maxWidth:860,margin:'0 auto',padding:'20px 24px'}}>
      ...
      {tab==='details'&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          ...
          <div style={card}>
            <div style={cardHd}>
              <div style={{fontSize:14,fontWeight:700}}>Contact &amp; insurance</div>
              {!onSaveInsurance&&<span style={{fontSize:11,fontWeight:600,color:'#8AA8A0'}}>read-only</span>}
            </div>
            <div style={{padding:'4px 18px 12px'}}>
              ...
              {onSaveInsurance?(
                <div style={{padding:'10px 0'}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    <div>
                      <div style={{fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',color:'#6A8880',marginBottom:4}}>Insurer</div>
                      <input value={insurer} onChange={e=>{setInsurer(e.target.value);setInsSaved(false);}} placeholder="Self-pay"
                        style={{width:'100%',fontSize:13,padding:'7px 10px',borderRadius:8,border:'1.5px solid #C8D4CF'}}/>
                    </div>
                    <div>
                      <div style={{fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',color:'#6A8880',marginBottom:4}}>Policy #</div>
                      <input value={policy} onChange={e=>{setPolicy(e.target.value);setInsSaved(false);}} placeholder="—"
                        style={{width:'100%',fontSize:13,padding:'7px 10px',borderRadius:8,border:'1.5px solid #C8D4CF'}}/>
                    </div>
                  </div>
                  <div style={{display:'flex',justifyContent:'flex-end',marginTop:8}}>
                    <button onClick={saveInsurance} disabled={!insuranceChanged&&!insSaved}
                      style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:9,border:'none',background:insSaved?'#0A6040':PRIMARY,color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',opacity:(!insuranceChanged&&!insSaved)?.4:1}}>
